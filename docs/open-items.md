...
areas** — a car-park layout of **blocks** of pallets with roads drawn between
them, addressed **Block · Row · Col**, a tappable map, and a "Can't scan? Pick
location" path for un-scannable pallets (0033–0035). Before that: the
...
### A1. Verify migrations 0016 → 0035 in production
...
**0033–0035 add pallet columns to `shelves`** (`pallet_block`/`pallet_row`/
`pallet_col`; `aisle_after` from 0034 is now unused) — generate a pallet area as
**blocks** (add blocks, set each block's rows × columns), place wood via **Can't
scan? Pick location** by tapping a pallet on the map, then find it by name and
confirm the coordinate (**Block · Row · Col**) shows; check ordinary shelves are
unchanged.
...
- **`src/lib/modules.ts` and the `modules` table must stay in sync.** The
  ...
  the server refuses.
- **Migrations run in order 0001 → 0035**, all idempotent. 0017's function
